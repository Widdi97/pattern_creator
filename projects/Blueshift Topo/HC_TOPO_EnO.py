...
def create_pattern_in_memory(index, overlap1, overlap2, x_offset, y_offset, start_x, start_y, spalten, diameter, resolution):
    result = []
    ...
    cc_dist2 = diameter * (overlap1 + overlap2)  # Correct use of cc_dist2
    ...
    for pattern_name, n_circles, n_subtract_circles, add_offset, subtract_offset, offsetx, offsety, corn_offsety in [
        ...
        (pattern_name5, 6, 0, 0, 0, repx * shiftx, 0, 0),
    ]:
    ...
    end_time = time.time()
    print(f"Pattern {index} took {end_time - start_time:.4f} seconds")
