...
overlappx_list = [0.7, 0.8, 0.9]
overlappy_list = [0.7, 0.8, 0.9]
resolution_ratio = 10
file_name = "tedrads_DO.pat"

with open(file_name, 'w') as file:
    for diameter in diameter_list:
        dia_string = f"{diameter}".replace(".","p")
        file.write("D tedrads_do_"+ dia_string + '\n' + "I 1" + '\n' + "C 100" + '\n')
        ...
        for overlappx in overlappx_list:
            for overlappy in overlappy_list:
                pn = f"teds_d{diameter}"
                pattern_name = pn.replace(".","p")
                print(pn)
                position = overlappx_list.index(overlappx)+3*overlappy_list.index(overlappy)
                
                # intitialize pattern object
                pattern = Pattern(diameter*(1+overlappx)*1e3+off, diameter*(1+overlappy)*1e3+off, resolution, pattern_name)
