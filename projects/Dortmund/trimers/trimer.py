...
with open(file_name, 'w') as file:
    for diameter in diameter_list:
        ...
        for overlapp in overlapp_list:
            ...
            pn = f"tri_d{diameter}_v{overlapp}_res{resolution}"
            pattern_name = pn.replace(".","p")
            print(pn)
            
            # intitialize pattern object
            pattern = Pattern(diameter*(1+overlapp)*1e3+2*off, (diameter*(np.sqrt(3)/2+overlapp))*1e3+off/2+2*off, resolution, pattern_name)

            # add all circles
            pattern.add_parametrized_shape(circle, diameter/2*1e3+off, (diameter*(np.sqrt(3)/2+overlapp-0.5))*1e3+off, diameter/2*1e3)
            pattern.add_parametrized_shape(circle, diameter*(.5+overlapp)*1e3+off, (diameter*(np.sqrt(3)/2+overlapp-0.5))*1e3+off, diameter/2*1e3)
            pattern.add_parametrized_shape(circle, (diameter*(1+overlapp))/2*1e3+off, diameter/2*1e3+off, diameter/2*1e3)
