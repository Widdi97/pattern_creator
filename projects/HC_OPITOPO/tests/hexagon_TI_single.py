...
with open(file_name, 'w') as file:
    ...
    for (overlapp1, overlapp2) in zip(overlapp1_list, overlapp2_list):
        ...
        pn1 = f"hexti_d{diameter}_va{overlapp1}_vb{overlapp2}_1,{round(2*(np.sqrt(3)*diameter*overlapp1 + np.sqrt(3)/2*diameter*overlapp2)*1000)},{round((2*diameter*overlapp1 + diameter*overlapp2)*1000)},5,5"
        pattern_name1 = pn1.replace(".","p")
        print(pn1)
        pn2 = f"hexti_d{diameter}_va{overlapp1}_vb{overlapp2}_2,{round(2*(np.sqrt(3)*diameter*overlapp1 + np.sqrt(3)/2*diameter*overlapp2)*1000)},{round((2*diameter*overlapp1 + diameter*overlapp2)*1000)},5,5"
        ...
        # generate .pat string
        pattern_string = pattern.export_pattern(complete=True, export=False, offsetx = round((np.sqrt(3)*diameter*overlapp1 + np.sqrt(3)/2*diameter*overlapp2)*1000), offsety = round(diameter*(overlapp1+0.5*overlapp2)*1000))
        file.write(pattern_string + '\n'+ '\n')
        nlines = pattern_string.count('\n') - 3
